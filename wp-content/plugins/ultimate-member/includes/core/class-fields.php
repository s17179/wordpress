					...
					if ( $key == 'single_user_password' ) {
						...
						if ( $this->set_mode != 'login' && isset( $data['force_confirm_pass'] ) && $data['force_confirm_pass'] == 1 ) {
							...
							if ( isset( $data['label'] ) ) {
								...
								$output .= $this->field_label( sprintf( __( 'Potwierdź %s', 'ultimate-member' ), $data['label'] ), $key, $data );
							}
							...
							$output .= '<input class="' . $this->get_class( $key, $data ) . '" type="' . esc_attr( $input ) . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $key . UM()->form()->form_suffix ) . '" value="' . $this->field_value( $key, $default, $data ) . '" data-validate="' . esc_attr( $validate ) . '" data-key="' . esc_attr( $key ) . '" />
